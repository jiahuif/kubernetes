...
func (o *LeaderMigrationOptions) ApplyTo(cfg *config.GenericControllerManagerConfiguration) error {
	...
	if o.ControllerMigrationConfig == "" {
		return loadDefaultLeaderMigrationConfiguration(&cfg.LeaderMigration)
	}
...
func loadDefaultLeaderMigrationConfiguration(cfg *config.LeaderMigrationConfiguration) error {
	versioned := new(v1alpha1.LeaderMigrationConfiguration)
	v1alpha1.RecommendedDefaultLeaderMigrationConfiguration(versioned)
	s := runtime.NewScheme()
	v1alpha1.SchemeBuilder.AddToScheme(s)
	return s.Convert(versioned, cfg, nil)
}
