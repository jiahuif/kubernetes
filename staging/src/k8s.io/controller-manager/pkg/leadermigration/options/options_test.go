...
func TestLeaderMigrationOptions(t *testing.T) {
	defaultConfig := new(config.LeaderMigrationConfiguration)
	loadDefaultLeaderMigrationConfiguration(defaultConfig)
	testCases := []struct {
		name              string
		flags             []string
		...
		{
			name:              "enabled, with default configuration file",
			flags:             []string{"--enable-leader-migration"},
